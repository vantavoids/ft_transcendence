...
app.AddCommand("listen", async (string url = DefaultChatHub, string? token = null, long[]? channel = null, int seconds = 60) =>
{
	...
	conn.On<object>("DirectMessageEdited",    m => Console.WriteLine($"<- DirectMessageEdited: {m}"));
	conn.On<object>("DirectMessageDeleted",   m => Console.WriteLine($"<- DirectMessageDeleted: {m}"));
	conn.On<object>("UserPresence",           m => Console.WriteLine($"<- UserPresence: {m}"));
	conn.On<string>("GuildLeft",              id => Console.WriteLine($"<- GuildLeft: {id}"));
	conn.On<string,
			string>("GuildJoined",   (id, name) => Console.WriteLine($"<- GuildJoined: {id} ({name})"));
	conn.On<string,
			string,
			string,
			DateTimeOffset>("TypingStarted", (userId, scope, id, until) => Console.WriteLine($"<- TypingStarted: user={userId} scope={scope} id={id} until={until}"));
...
app.AddCommand("session", async (string url = DefaultChatHub, string? token = null) =>
{
	...
	conn.On<object>("UserPresence",         m => Console.WriteLine($"<- UserPresence: {m}"));
	conn.On<string>("GuildLeft",            id => Console.WriteLine($"<- GuildLeft: {id}"));
	conn.On<string, string>("GuildJoined",  (id, name) => Console.WriteLine($"<- GuildJoined: {id} ({name})"));
	conn.On<string, string, string, DateTimeOffset>("TypingStarted",
...
}).WithDescription("interactive session: stay connected and send hub invocations (join, leave, typing)");
