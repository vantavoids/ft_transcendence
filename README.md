...
The Notification Service is a passive consumer. Almost all of its input arrives from the broker, which means I needed real control over what happens to each message, so I disabled auto-ack and took on the responsibility of acking myself. Not every failure deserves the same outcome, for example a transient one, if the databse is down or the user service in unreachable, this should be nacked and requeued, while a malformed payload is a poison message that will loop forever if I put it back. Drawing that line wasn't a decision I could make alone, because the events come from my teammates' services, so the taxonomy was arbitrated with the team. The service also has no deduplication, and that's an identified limit rather than an oversight, if the mail fails to send after the notification has already been persisted, a redelivery recreates it. There is more than one flaw in my service, I know where the they are and why they exist, but it fell outside the scope we could reasonably cover, they are written in my service so we may one day come back and implement/fix them.

The contract originally called for a SignalR WebSocket for real-time delivery, and I pushed back, a notification stream is unidirectional, and paying for a full duplex connection to send in one direction is cost without benefit, so I proposed Server-Sent Events instead, and the change was accepted quickly.
But is brought its own problem, the consumer runs on its own goroutine, each SSE connection is another goroutine blocked on its ResponseWriter, and routing a notification from one to the other means keeping a registry of live connections per user, fanning out to every open tab, and cleaning up on r.Context().Done() when the client disconnects, or the goroutines and channels leak. 
Authentication had to move to the query string, since the browser's EventSource API won't let you set custom headers. SSE is a layer on top of the database, not dependent on it, a user who's offline loses nothing, the notification is persisted and picked up on the next fetch.

I chose sqlc over an ORM, the SQL is written by hand and sqlc generates typed Go from it, so there's no change-tracking behaviour to be surprised by and no ambiguity about which query actually runs. That paid off on the GDPR deletion, which has to clear a user's notifications and their preferences together, either both tables are cleaned or neither is. Wrapping both in one transaction with the rollback deferred as a safety net made that guarantee cheap to express and easy to read. Notification preferences are scoped to guilds and channels only, which is a deliberate limit rather than an unfinished feature, and old notifications are purged after seven days by a ticker running in its own goroutine.

That restraint was itself part of the work. The consumer runs on a single goroutine, and the design allows more if the volume ever calls for it, but nothing in what we actually do calls for it today. The fan-out is bounded by construction: a mention produces one notification per mentioned member, not one per member of the channel, so the work generated by any single event stays small and predictable. Deciding what genuinely needed to scale, what could scale later, and what didn't need to scale at all turned out to be a real part of building this service, and resisting the temptation to over-engineer the parts that didn't need it was harder than it sounds.

Almost everything about this project was new to me at the start: Go, RabbitMQ and, more importantly how to work as one person inside a group project.
The real challenge was never the stack. Go itself turned out to be a good place to learn backend work: simple to write, easy to read, and well suited to it thanks to the goroutines net/http spawns per request. What took longer was learning to think about a service as one piece of a system I didn't control, and to work with a team through issues, pull requests and shared conventions rather than around them.